    ...
    if (existingGroupIndex !== -1) {
      ...
      updatedRules[existingGroupIndex] = {
        ...
        rules: existingGroup.rules
          //explicitly assign parentId to every rule inside the group
          .map((rule) => ({
            ...rule,
      ...
      //if the correct gender rules doesn't exists inside the group, add it 
      if (!ruleExists) {
        updatedRules[existingGroupIndex] = {
          ...updatedRules[existingGroupIndex],
          rules: [
            ...updatedRules[existingGroupIndex].rules,
            //the new rule is added only if it didn't exists before 
            {
              id: uuidv4(),
              field: newGender === "male" ? "sports" : "cosmetics",
              operator: "=",
              value: "",
              parentId: existingGroup.id, // Correctly associate the rule with its parent
            },
          ],
        };
      }
